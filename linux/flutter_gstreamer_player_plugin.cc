    ...
    FlMethodCall *method_call)
{
  ...
  else if (strcmp(method, "PlayerRegisterTexture") == 0)
  {
    ...
    if (added)
    {
      ...
      gstPlayer->onVideo([texture_registrar = self->texture_registrar,
                          ...
                          video_outlet_private = video_outlet_private](uint8_t *frame, uint32_t size, int32_t width, int32_t height, int32_t stride) -> void
                         {
  ...
  else if (strcmp(method, "isPlaying") == 0)
  {
    ...
    int32_t player_id = fl_value_get_int(fl_value_lookup_string(arguments, "playerId"));
    GstPlayer *gstPlayer = g_players->Get(player_id);
    bool playing = gstPlayer->isPlaying();
  ...
  else if (strcmp(method, "seekTo") == 0)
  {
    ...
    int32_t player_id =
        fl_value_get_int(fl_value_lookup_string(arguments, "playerId"));
    int64_t position_ms =
        fl_value_get_int(fl_value_lookup_string(arguments, "position"));
    GstPlayer *gstPlayer = g_players->Get(player_id);
    gstPlayer->seekTo(position_ms);
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(nullptr));
  }
